...
| Dependencies | pdfplumber, pypdf, reportlab, qpdf, pytesseract, pdf2image, Tesseract, poppler | `pypdfium2` (plus `pillow` for rendering) |
...
| Anything unexpected — unreadable or password-protected file, zero pages, no engine, only `pypdf` when a render is needed, hook crash | Allow. A hook must never be the reason a Read fails. |

If the extraction is longer than one Read shows (over 1,800 lines or about 40,000 tokens), the rewritten Read returns the first 200 lines and the note says so, pointing at `Grep` or `offset`/`limit` rather than handing back a silent prefix. A Read with `pages="100-105"` on any text PDF lands on those pages: the hook sets `offset`/`limit` to the span between their `[page N/M]` markers. Your own `offset`/`limit`, if you pass them, are left alone.
...
Python 3.9 or newer (CI runs 3.9 and 3.12). `pillow` is only needed for the rendering path — if you never read a scan without poppler, you never need it.
...
(If your plugin root is elsewhere, `/plugin` shows the installed path.) The self-test reports the active engine, whether rendering is available, whether `pdftoppm` is on your PATH, the mode, and the cache directory with its usage and limits. `--check FILE.pdf` prints the routing decision for one file without side effects.
...
**Engines.** `pypdfium2` (BSD-3-Clause / Apache-2.0) is the default. `pymupdf` is used if already installed, but is never a dependency — it is AGPL-3.0, which is a problem for exactly the people who care most about token cost. `pypdf` is a last resort: it cannot render, and its extraction does not agree with `pypdfium2`'s on complex layouts — on the four CJK files in the corpus its CJK character count differs from `pypdfium2`'s by −0.8% to +17% (`pypdf` 6.13), so a verdict or a cache built by one engine is not interchangeable with the other's.
